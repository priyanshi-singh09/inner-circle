...
public class ExploreService {
    ...
    @Transactional(readOnly=true) public Page<ExploreUserResponse> people(UUID viewerId,String query,Pageable pageable){User viewer=activeUser(viewerId);String filter=query==null||query.isBlank()?null:query.trim();return userRepository.findExplorePeople(viewerId,viewer.getCircle().getId(),filter,pageable).map(this::toUserResponse);}
