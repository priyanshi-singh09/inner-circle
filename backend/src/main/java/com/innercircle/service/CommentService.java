...
public class CommentService {
    ...
    @Transactional(readOnly=true) public Page<CommentResponse> getForPost(UUID viewerId,UUID postId,Pageable pageable){if(!postRepository.existsById(postId))throw new IllegalArgumentException("Post not found.");return commentRepository.findVisibleComments(postId,viewerId,pageable).map(this::toResponse);}
