...
function App() {
  ...
  return (
    <div className="app-shell">
      <header className="topbar"><button className="icon-button" aria-label="Open menu">☰</button><div className="brand"><span className="brand-mark">◉</span><span>Inner Circle</span></div><button className="icon-button" aria-label="Notifications">🔔</button></header>
      <main className="main-content">
        ...
        <section className="emotion-section"><div className="section-heading"><h2>How are you feeling?</h2><button>Explore all</button></div><div className="emotion-row">{emotions.map(([emoji, label]) => <button className="emotion-chip" key={label}><span>{emoji}</span><small>{label}</small></button>)}</div></section>
      ...
      <nav className="bottom-nav" aria-label="Main navigation">{[['🏠','Feed'],['🔎','Explore'],['＋','Share'],['🤝','Connections'],['👤','Profile']].map(([icon,label]) => <button className={screen === label ? 'nav-active' : ''} key={label} onClick={() => setScreen(label)}><span>{icon}</span><small>{label}</small></button>)}</nav>
    </div>
  );
}
export default App;
